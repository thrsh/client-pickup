// Tab shell for the approver area: "pickup" (PickupApprovalsTab) and
// "stale" (ApproverStaleReports, lazy-loaded on first activation). Both
// tabs stay mounted after first activation (hidden via CSS) so their
// internal filter/search state survives switching. Role gating is
// enforced here AND one level up by <ProtectedRoute>.
...
// Syncs the active tab to ?tab= without a router dependency.
...
class TabErrorBoundary extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-orange-200 bg-orange-50 px-4 py-16 text-center">
        <AlertTriangle className="h-8 w-8 text-orange-400" />
        <p className="font-display text-lg font-semibold text-ink-700">This tab hit an error</p>
        <p className="max-w-sm text-sm text-ink-400">{this.state.error?.message || 'Something went wrong rendering this tab.'}</p>
        <button onClick={this.handleRetry} className="rounded-md border border-ink-200 px-3.5 py-1.5 text-xs font-medium text-ink-600 hover:bg-ink-50">
          Try again
        </button>
      </div>
    )
...
// Keeps content mounted (never unmounted) across tab switches; only the
// fade-in transition below is tied to `active`.
function TabPanel({ active, children }) {
  const [entered, setEntered] = useState(false)
  useEffect(() => {
    if (!active) return setEntered(false)
    if (prefersReducedMotion()) return setEntered(true)
  ...
  return (
    <div className={!active ? 'hidden' : undefined}>
      <div className="tab-panel-fade" style={{ opacity: entered ? 1 : 0, transform: entered ? 'translateY(0)' : 'translateY(4px)' }}>
        {children}
      </div>
    </div>
  )
    ...
    <div>
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .tab-indicator-transition, .tab-panel-fade { transition: none !important; }
        }
      `}</style>
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-dashed border-ink-100 pb-4">
        <div ref={tabListRef} role="tablist" className="relative flex items-center gap-1 rounded-lg border border-ink-100 bg-ink-50/60 p-1 shadow-sm">
          <span
            aria-hidden="true"
            className={cn('absolute inset-y-1 rounded-md bg-white shadow-sm ring-1 ring-ink-100', indicatorTransitionsEnabled && 'tab-indicator-transition')}
          ...
          {TAB_CONFIG.map(({ key, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                onClick={() => setTab(key)}
                className={cn('relative z-10 flex items-center gap-2 rounded-md px-3.5 py-2 text-sm font-medium transition-colors', isActive ? 'text-ink-900' : 'text-ink-500 hover:text-ink-700')}
              >
                <Icon className={cn('h-4 w-4', isActive ? 'text-teal-600' : 'text-ink-300')} />
                {label}
                {meta.count > 0 && (
                  <span
                    className={cn(
                      'rounded-full px-1.5 py-0.5 text-[10px] font-semibold transition-transform',
                      isActive ? 'bg-teal-100 text-teal-700' : 'bg-ink-200 text-ink-600',
                      flashing && 'scale-110'
                    )}
                  >
                    {meta.count}
        ...
        <button onClick={bumpRefresh} className="flex items-center gap-1.5 rounded-md border border-ink-200 px-3 py-2 text-sm font-medium text-ink-600 hover:bg-ink-50" title="Refresh both tabs">
